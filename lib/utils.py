...
def get_dataset(args, n_list, k_list, classes_list):
    ...
    if args.dataset == 'mnist':
        ...
        if args.iid:
            ...
            if args.unequal:
                ...
                # Chose euqal splits for every user
                user_groups, classes_list = cifar100_noniid(args, train_dataset, args.num_users, n_list, k_list, classes_list)
                user_groups_lt = cifar100_noniid_lt(args, test_dataset, args.num_users, classes_list)
...
def compute_class_accuracy(y_true, y_pred, num_classes):
    ...
    for i in range(num_classes):
        if class_total[i] > 0:
            class_accuracy[i] = float(class_correct[i]) / class_total[i]
...
if __name__ == '__main__':
    ...
    all_class_list = np.random.choice(np.arange(1, 100), size=args.num_classes, replace=False)
